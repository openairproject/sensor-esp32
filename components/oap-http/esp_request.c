...
#define REQ_CHECK(check, log, ret) if(check) {ESP_LOGW(TAG, log);ret;}
...
static int nossl_connect(request_t *req)
{
    ...
    socket_esp = socket(PF_INET, SOCK_STREAM, 0);
    REQ_CHECK(socket_esp < 0, "socket failed", return -1);
    ...
    tv.tv_usec = 0;
    setsockopt(socket_esp, SOL_SOCKET, SO_RCVTIMEO, &tv, sizeof(tv));

    ESP_LOGD(TAG, "[sock=%d],connecting to server IP:%s,Port:%s...",
             socket_esp, ipaddr_ntoa((const ip_addr_t*)&remote_ip.sin_addr.s_addr), (char*)port->value);
    if(connect(socket_esp, (struct sockaddr *)(&remote_ip), sizeof(struct sockaddr)) != 0) {
        close(socket_esp);
        return -1;
    }
    req->socket = socket_esp;
    return socket_esp;
}
...
static int mbedtls_connect(request_t *req)
{
    ...
    do {
        ...
        if ((ret = mbedtls_ctr_drbg_seed(&ssl->drbg_ctx, mbedtls_entropy_func,
        	...
        	ESP_LOGW(TAG, "mbedtls_ctr_drbg_seed returned %d", -ret);
            break;
        }
        ...
        */
        ...
        if (req->client_cert && req->client_key) {
            if ((ret = mbedtls_ssl_get_record_expansion(&ssl->ssl_ctx)) >= 0) {
            	ESP_LOGV(TAG, "Record expansion is %d", ret);
            } else {
                ESP_LOGV(TAG, "Record expansion is unknown (compression)"); //not error
            }
        }
...
static int mbedtls_write(request_t *req, char *buffer, int len)
{
    ...
    while ((ret = mbedtls_ssl_write(&req->ssl->ssl_ctx, (unsigned char *)buffer, len)) <= 0) {
        if (ret != MBEDTLS_ERR_SSL_WANT_READ && ret != MBEDTLS_ERR_SSL_WANT_WRITE && ret != -76) {
        	ESP_LOGW(TAG, "mbedtls_ssl_write returned 0x%x", ret);
            break;
        }
    }
    return ret;
...
static int mbedtls_read(request_t *req, char *buffer, int len)
{
	...
	do {
		...
		if (ret == MBEDTLS_ERR_SSL_WANT_READ || ret == MBEDTLS_ERR_SSL_WANT_WRITE) {
			...
			ESP_LOGD(TAG, "peer close"); //this is expected for Connection: close
			break;
		} else {
			break;
		}
...
int req_perform(request_t *req)
{
    do {
        ...
        REQ_CHECK(req->_connect(req) < 0, "error connect", break);
